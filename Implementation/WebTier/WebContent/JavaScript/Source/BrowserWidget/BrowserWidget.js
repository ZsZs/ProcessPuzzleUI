...
var BrowserWidget = new Class({
   ...
   Binds: 'webUIMessageHandler',
	...
	initialize: function ( options, resourceBundle ) {
      ...
      this.loadWidgetDefinition();
		...
		AssertUtil.assertParamNotNull( this.i18Resource, "resourceBundle" );
		if( this.containerElement == null ) throw new IllegalArgumentException( "Parameter 'widgetContainerId' in invalid." );
		if( !this.i18Resource.isLoaded ) throw new IllegalArgumentException( "ResourceBundle should be already loaded." );;
	},
   ...
   webUIMessageHandler: function( webUIMessage ){
	   if( !this.isConfigured ) throw new UnconfiguredWidgetException({ source : 'BrowserWidget.webUIMessageHandler()' });
   ...
   getMessageBus: function() { return this.messageBus; },
	...
	configureLogger : function() {
		if( this.webUIController == null ){
			...
			if( this.logger == null ) this.logger = new WebUILogger();
		}
		else
   ...
   loadWidgetDefinition: function() {
   	if( this.options.widgetDefinitionURI ){
      	try{
      		this.definitionXml = new XmlResource( this.options.widgetDefinitionURI, { nameSpaces : this.options.definitionXmlNameSpace } );
      	}catch( e ){
